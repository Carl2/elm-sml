...
<script>
    ...
    app.ports.openCompilerExplorer.subscribe(function(code) {
        ...
        // Using a more flexible regex to handle potential whitespace variations
        ...
        if (code === godboltCode) {
            console.log("WARNING: No replacement made! Looking for '#include <boost/sml.hpp>' in code...");
            console.log("Code contains 'boost/sml.hpp':", code.indexOf('boost/sml.hpp') !== -1);
            console.log("Code contains '#include':", code.indexOf('#include') !== -1);
            // Try to find what the include line looks like
            var lines = code.split('\n');
            lines.forEach(function(line, i) {
                if (line.indexOf('include') !== -1) {
                    console.log("Line " + i + " with include:", JSON.stringify(line));
                }
            });
        } else {
